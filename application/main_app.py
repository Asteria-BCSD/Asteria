...
class Asteria():
    ...
    def __init__(self, checkpoint_path, model_selector, cuda=False):
        # cuda = True
        self.dh = DataHelper()  # For database accessing.
    ...
    def ast_encode_similarity(self, sources=[], targets=[], threshold=0):
        ...
        for function_name, elf_path, ast_encode in tqdm(sources):
            ...
            for tfunction_name, telf_path, tast_encode in tqdm(targets, desc="func %s" % function_name):
                ...
                res.append((pool.apply_async(self.compute_app.similarity_vec,
    ...
    def ast_similarity(self, sources=[], targets=[], astfilter=None, threshold=0):
        ...
        for s_func_info, s_ast in sources:
            ...
            res.sort(key=lambda x: x[1], reverse=True)  # sort
...
def parse_args():
    ...
    ap.add_argument("--model_selector", type=str,
                    default="treelstm", choices=['treelstm', 'binarytreelstm', 'lstm'])
    ap.add_argument("--output", type=str,
                    default="VulSearch.result", help="file path to save search results")
...
def treelstm_search():
    ...
    if threshold < 0 or threshold > 1:
        print("Threshold Value Error! The range is 0~1!")
        exit(1)
